...
def test_tray_mode_must_not_swallow_the_exit_during_an_update():
    """With "minimize to tray" on, closing the window normally just HIDES it — the
    process survives and keeps the .exe locked, so the updater's swap never lands and
    the helper relaunches the OLD exe. An update must genuinely exit."""
    from exilebot_pickit.webui.poc import _should_hide_instead_of_exit
    ...
    tray = object()
    a = FakeApi()
    assert _should_hide_instead_of_exit(a, tray) is True      # normal close -> hide

    a._quitting = True                                        # updating
    assert _should_hide_instead_of_exit(a, tray) is False     # must really exit

    b = FakeApi()
    b.cfg = {"minimize_to_tray": False}
    assert _should_hide_instead_of_exit(b, tray) is False
    assert _should_hide_instead_of_exit(FakeApi(), None) is False   # no tray at all


def test_install_update_marks_the_app_as_quitting(api, tmp_path, monkeypatch):
    """install_update must set the flag BEFORE it returns, or the close it triggers
    races the tray handler and gets swallowed."""
    import subprocess
    import sys as _sys
    new_exe = tmp_path / "new.exe"
    new_exe.write_bytes(b"MZ")
    api._dl = {"result": {"ok": True, "path": str(new_exe)}}
    api.cfg["minimize_to_tray"] = True
    monkeypatch.setattr(_sys, "frozen", True, raising=False)
    monkeypatch.setattr(_sys, "executable", str(tmp_path / "cur.exe"), raising=False)
    monkeypatch.setattr(subprocess, "Popen", lambda *a, **k: object())
    monkeypatch.setattr(threading, "Thread",
                        lambda *a, **k: type("T", (), {"start": lambda s: None})())
    assert api.install_update().get("ok")
    assert getattr(api, "_quitting", False) is True
